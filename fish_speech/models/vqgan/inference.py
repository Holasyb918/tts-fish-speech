...
def load_model(config_name, checkpoint_path, device="cuda"):
    ...
    state_dict_0 = {}
    for k, v in state_dict.items():
        # print(k.replace(".parametrizations.weight.original0", ".weight_g"), k.replace(".parametrizations.weight.original1", ".weight_v"))
        state_dict_0[k.replace(".parametrizations.weight.original0", ".weight_g").replace(".parametrizations.weight.original1", ".weight_v")] = v
    result = model.load_state_dict(state_dict_0, strict=True)
    model.eval()
    model.to(device)
